<x-app-layout>
    <div x-data="{ open: false, setting: null }">
                                    ...
                                    <tr class="border-t">
                                        ...
                                        <td class="px-3 py-2">
                                            ...
                                            @elseif ($setting->type === 'json')
                                                <pre class="bg-gray-100 p-2 rounded text-xs">{{ json_encode(json_decode($setting->value, true), JSON_PRETTY_PRINT) }}</pre>
                                            @else
        ...
        {{-- MODAL --}}
        <div x-show="open" 
            class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-80 p-6 z-50">
            ...
            <div x-show="open" x-transition.duration.500ms
                ...
                <form method="POST" :action="setting.id ? '/admin/landing/settings/' + setting.id : '{{ route('admin.landing.settings.store') }}'"
                    ...
                    <div class="mb-4">
                        <label class="block font-medium mb-1 text-white">Key</label>
                    ...
                    <template x-if="setting.type === 'image'">
                        <div class="mb-4">
                            <label class="block font-medium mb-1 text-white">Upload Image</label>
                            <input type="file" name="value" class="w-full border rounded px-3 py-2">
                            
                            <template x-if="setting.value && setting.id">
                                <div class="mt-2">
                    ...
                    <template x-if="setting.type !== 'image'">
                        <div class="mb-4">
                            <label class="block font-medium mb-1 text-white">Value</label>
                            <textarea name="value" rows="5" x-model="setting.value"
